...
function applyAndRender() {
  applyFilter(reddify), applyFilterNoBackground(decreaseBlue), applyFilterNoBackground(increaseGreenByBlue), render(
...
function applyFilter(e) {
  for (const n of image) {
	 for (let r = 0; r<n.length; r++) {
		let t = n[r],i = rgbStringToArray(t);
...
function applyFilterNoBackground(e) {
  let n = image[0][0];
  for (const r of image) {
	 for (let t = 0; t<r.length; t++) {
		if (r[t]!==n) {
		  ...
		  e(i), n = rgbArrayToString(i), r[t] = n
		}
	 }
  }
}
