...
dataShapes.push({color:"blue",shape:"circle",repeat:3})
  ...
  const 
handleStatic   = a => {setBackgroundWithObject(a), animationDetails.displayType = 1},
...
handleBad      = (a, i) => {i++, setBackgroundWithMixed(a, i), animationDetails.displayType = 3}
  ...
  function generateShapeData() {
    ...
    for (let c of colors) {
      for (let s of shapes) {
        for (let r of repeats) {data.push({color: c,repeat: r,shape: s})}
      }
    }
    return data;
  }
  ...
  function setBackgroundWithObject(obj) {"tesseract" === obj.shape ? $("#shape").css("background", `url(images/${obj.shape}.gif)`) : $("#shape").css("background", `url(images/${obj.color}-${obj.shape}.png)`), setBackgroundRepeat(obj.repeat)}
  function setBackgroundWithSimple(color,shape,repeat) {$("#shape").css("background", `url(images/${color}-${shape}.png)`), setBackgroundRepeat(repeat)}
  function setBackgroundWithMixed(obj,repeat) {$("#shape").css("background", `url(images/${obj.color}-${obj.shape}.png)`), setBackgroundRepeat(repeat)}
...
function animateBounce() {$("#shape").css("background-size", `${100/repeat}% ${100/repeat}%`) aanimationDetails.x += animationDetails.speedX, animationDetails.y += animationDetails.speedY, (animationDetails.x + $("#shape").width() + 8 >= $("#shape-container").width() || animationDetails.x < 2) && (animationDetails.speedX *= -1), (animationDetails.y + $("#shape").height() + 4 >= $("#shape-container").height() || animationDetails.y < 2) && (animationDetails.speedY *= -1), $("#shape").css("left", animationDetails.x), $("#shape").css("top", animationDetails.y)}
function animateBlink() {animationDetails.showCount--, 0 === animationDetails.showCount && (animationDetails.show = !animationDetails.show, animationDetails.show ? $("#shape").css("display", "block") : $("#shape").css("display", "none"), animationDetails.showCount = 60)}
