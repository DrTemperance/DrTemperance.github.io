...
function getPosition(bone, matrixWorldInv) {return _vector.setFromMatrixPosition(bone.matrixWorld).applyMatrix4(matrixWorldInv);}

function setPositionOfBoneToAttributeArray(array, index, bone, matrixWorldInv) {
	array[index * 3] = getPosition(bone, matrixWorldInv).x;
	array[index * 3 + 1] = getPosition(bone, matrixWorldInv).y;
	array[index * 3 + 2] = getPosition(bone, matrixWorldInv).z;
}
	...
	updateMatrixWorld(force) {
		...
		if (this.visible) {
			...
			for (let i = 0, il = this.iks.length; i<il; i++) {
				const ik           = (this.iks)[i],
				      targetBone   = (mesh.skeleton.bones)[ik.target],
				      effectorBone = (mesh.skeleton.bones)[ik.effector],
				      targetMesh   = this.children[offset++],
				      effectorMesh = this.children[offset++];
				...
				for (let j = 0, jl = ik.links.length; j<jl; j++) {
					const link = ik.links[j], linkMesh = this.children[offset++];
					linkMesh.position.copy(getPosition((mesh.skeleton.bones)[link.index], _matrix));
				}
				...
				for (let j = 0, jl = ik.links.length; j<jl; j++) {
					...
					const linkBone = (mesh.skeleton.bones)[link.index];
