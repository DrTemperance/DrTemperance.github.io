(window => {
	  ...
	  window.opspark.createPlayerManager = (player,game) => {
		 ...
		 const update = () => {
			...
			if (game.cursors.left.isDown || aKey.isDown) {player.setDirection(-1), player.run()}
			if (game.cursors.right.isDown || dKey.isDown) {player.setDirection(1), player.run()}
			if (game.cursors.down.isDown || sKey.isDown) {player.duck()}
			if (game.cursors.up.isDown || wKey.isDown) {player.asset.body.touching.down && player.flyingJump()}
		 };
